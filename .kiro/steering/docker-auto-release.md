...
Reglas obligatorias para cualquier cambio en el repositorio:

1. **Nunca trabajar directamente en `main`** — siempre crear una rama con prefijo descriptivo (`feat/`, `fix/`, `docs/`, `chore/`)
2. **Todo cambio va por Pull Request** — crear PR con título y descripción claros
3. **Merge con squash** — usar `gh pr merge --squash --delete-branch` para mantener el historial limpio
4. **Borrar ramas después del merge** — tanto la local como la remota deben eliminarse (el flag `--delete-branch` se encarga de ambas)
5. **Validar releases** — después de disparar el workflow, verificar que:
   - El workflow completó exitosamente: `gh run view <run-id>`
   - El release se creó correctamente: `gh release view <tag>`
   - La imagen existe en Docker Hub con los tags esperados

## Convenciones de commits

- Todos los mensajes de commit deben estar en **inglés**
- Usar prefijos convencionales: `feat:`, `fix:`, `docs:`, `chore:`, `refactor:`, `ci:`
- Ejemplo: `feat: add multi-arch CI/CD workflow`
